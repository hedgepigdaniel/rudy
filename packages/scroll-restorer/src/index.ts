/* eslint-env browser */
import ScrollBehavior, { TransitionHook, ScrollPosition, ShouldUpdateScroll } from 'scroll-behavior'
import {
  Api,
  Middleware,
  LocationEntry,
...
export class RestoreScroll<Action extends FluxStandardRoutingAction> {
  private options: RestoreScrollOptions<Action>

  private behavior: ScrollBehavior<LocationEntry<Action>, Request<Action>, never>
...
>(options?: RestoreScrollOptions<Action>): ScrollRestorerCreator<Action, RestoreScroll<Action>> =>
  (api) => new RestoreScroll(api, options)
