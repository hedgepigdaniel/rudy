...
import { serverRedirect, pathlessRoute, anonymousThunk, transformAction, call, enter, changePageTitle, saveScroll, restoreScroll } from '../middleware';
export default (function () {
  ...
  var location = options.location,
      ...
      createRestoreScroll = _options$createRestor === void 0 ? defaultCreateRestoreScroll() : _options$createRestor; // assign to options so middleware can override them in 1st pass if necessary
  ...
  var scrollRestorer = isServer() ? undefined : createRestoreScroll(api);
  api.scrollRestorer = scrollRestorer;

  if (!middlewares) {
    middlewares = [serverRedirect, // short-circuiting middleware
    ...
    call('beforeLeave', {
      ...
      runOnServer: true
    }), saveScroll, enter, changePageTitle({
      title: options.title
    }), call('onLeave', {
      prev: true
    }), call('onEnter', {
